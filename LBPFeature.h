...
namespace LinLib
{
	...
	class LBPFeature
	{
		...
		cv::Mat& Calculate(const cv::Mat &_image, int nan_value=-1)
		{
			...
			//handle borders
			//current implementation does not handle 0 as separate values from NaN
			//solution: replace zeros with equivalent padding pixels
			switch (border_type)
			{
			case BORDER_PADD:
				_pd = (float*)feature.data;
				...
				if (image.type() == CV_8UC1)
				{
					...
					for(int j = 1; j < image.cols-1; j++)
						_pd[CalcIndex(_p[j], _p[j+1], _p2[j+1], _p2[j], _p2[j-1], _p[j-1], 0, 0, 0, nan_value)]++;

					//top corner points
					_pd[CalcIndex(_p[0], _p[1], _p2[1], _p2[0], 0, 0, 0, 0, 0, nan_value)]++;
					_pd[CalcIndex(_p[image.cols-1], 0, 0, _p2[image.cols-1], _p2[image.cols-2], _p[image.cols-2], 0, 0, 0, nan_value)]++;

					//last row
					_p1 = image.ptr<uchar>(image.rows-2);
					_p = image.ptr<uchar>(image.rows-1);
					for(int j = 1; j < image.cols-1; j++)
						_pd[CalcIndex(_p[j], _p[j+1], 0, 0, 0, _p[j-1], _p1[j-1], _p1[j], _p1[j+1], nan_value)]++;

					//bottom corner points
					_pd[CalcIndex(_p[0], _p[1], 0, 0, 0, 0, 0, _p1[0], _p1[1], nan_value)]++;
					_pd[CalcIndex(_p[image.cols-1], 0, 0, 0, 0, _p[image.cols-2], _p1[image.cols-2], _p1[image.cols-1], 0, nan_value)]++;
					...
					for(int i = 1; i < image.rows-1; i++)
					{
						...
						_p1 = image.ptr<uchar>(i-1);
						_p2 = image.ptr<uchar>(i+1);
						_pd[CalcIndex(_p[0], _p[1], _p2[1], _p2[0], 0, 0, 0, _p1[0], _p1[1], nan_value)]++;
						_pd[CalcIndex(_p[image.cols-1], 0, 0, _p2[image.cols-1], _p2[image.cols-2], _p[image.cols-2], _p1[image.cols-2], _p1[image.cols-1], 0, nan_value)]++;
					}
				}
				else if (image.type() == CV_16UC1)
				{
					...
					for(int j = 1; j < image.cols-1; j++)
						_pd[CalcIndex(_pp[j], _pp[j+1], _pp2[j+1], _pp2[j], _pp2[j-1], _pp[j-1], 0, 0, 0, nan_value)]++;

					//top corner points
					_pd[CalcIndex(_pp[0], _pp[1], _pp2[1], _pp2[0], 0, 0, 0, 0, 0, nan_value)]++;
					_pd[CalcIndex(_pp[image.cols-1], 0, 0, _pp2[image.cols-1], _pp2[image.cols-2], _pp[image.cols-2], 0, 0, 0, nan_value)]++;

					//last row
					_pp1 = image.ptr<ushort>(image.rows-2);
					_pp = image.ptr<ushort>(image.rows-1);
					for(int j = 1; j < image.cols-1; j++)
						_pd[CalcIndex(_pp[j], _pp[j+1], 0, 0, 0, _pp[j-1], _pp1[j-1], _pp1[j], _pp1[j+1], nan_value)]++;

					//bottom corner points
					_pd[CalcIndex(_pp[0], _pp[1], 0, 0, 0, 0, 0, _pp1[0], _pp1[1], nan_value)]++;
					_pd[CalcIndex(_pp[image.cols-1], 0, 0, 0, 0, _pp[image.cols-2], _pp1[image.cols-2], _pp1[image.cols-1], 0, nan_value)]++;
					...
					for(int i = 1; i < image.rows-1; i++)
					{
						...
						_pp1 = image.ptr<ushort>(i-1);
						_pp2 = image.ptr<ushort>(i+1);
						_pd[CalcIndex(_pp[0], _pp[1], _pp2[1], _pp2[0], 0, 0, 0, _pp1[0], _pp1[1], nan_value)]++;
						_pd[CalcIndex(_pp[image.cols-1], 0, 0, _pp2[image.cols-1], _pp2[image.cols-2], _pp[image.cols-2], _pp1[image.cols-2], _pp1[image.cols-1], 0, nan_value)]++;
					}
				}
				break;
			case BORDER_NAN:
